...
public class NewsDetailView extends Fragment{
    ...
    private void initView()
    {
    	...
    	bv_comment = new BadgeView(getActivity(), mCommentList);
    	...
    	//TODO 
    	//编辑器添加文本监听
    	//mFootEditer.addTextChangedListener(UIHelper.getTextWatcher(getActivity(), tempCommentKey));
    	
    	//TODO 
    	//显示临时编辑内容
    	//UIHelper.showTempEditContent(getActivity(), mFootEditer, tempCommentKey);
    }
    ...
    //初始化控件数据
       ...
       {    	
       	...
       	lvCommentAdapter = new ListViewCommentAdapter(getActivity(), lvCommentData, R.layout.comment_listitem); 
   		...
   		new Thread(){
   			public void run() {
   				...
   				if(action == UIHelper.LISTVIEW_ACTION_REFRESH || action == UIHelper.LISTVIEW_ACTION_SCROLL)
   					isRefresh = true;
   				try {
   					CommentList commentlist = ((AppContext)getActivity().getApplication()).getCommentList(catalog, id, pageIndex, isRefresh);				
